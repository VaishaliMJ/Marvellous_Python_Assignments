"""-----------------------------------------------------------------------------------------------------
                          Assignment20_4
                      DirectoryDuplicateRemovalTime.py
                    (Student name - Vaishali Jorwekar)
                     Python by Marvellous Infosystems
---------------------------------------------------------------------------------------------------------
Problem statement: Automation script  : DirectoryDuplicateRemovalTime.py
                   1.Design Automation script Which accepts directory from command line
                   2.Display duplicate files and delete them from the directory
                   3.Calculates execution time of the script     
---------------------------------------------------------------------------------------------------------""" 
    ...
    try:
        ...
        if(len(sys.argv)==1):
            ...
            if((sys.argv[1]=="--h") or (sys.argv[1]=="--H")):
                ...
                print("This Script searches duplicate files,delete and caculates execution time ")
            elif((sys.argv[1]=="--u") or (sys.argv[1]=="--U")):
                print("Use the given script as ")  
                print("ScriptName.py  directoryName")  
                print("Please provide valid absolute path....")
                print("\nRefer DirectoryDuplicateRemovalTime_log_{timestamp}.txt for output or error messages...")
            else:
                  ...
                  findAndRemoveDuplicateFiles(sys.argv[1],logFileName,ScriptStartTime)
        else:
            Module.invalidArgsMsg()
       #print(Border)
...
#-------------------------------------------------------------------------------         
def findAndRemoveDuplicateFiles(directoryName,logFileName,ScriptStartTime):
      try:
