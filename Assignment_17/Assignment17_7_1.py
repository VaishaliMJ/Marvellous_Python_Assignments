...
def generateBackupFolder(backupFileName):
    print("\n In generate backup folder name....")
...
def backupSourceFile(backupFileName,backupDirectoryName,logFileName):
    try:
        print(backupSourceFile)
        subFolderPath=generateBackupFolderDaily(backupDirectoryName)
        destiFileName=generateBackUpFileName(backupFileName)
        subFolderPath=os.path.realpath(subFolderPath)
        #print("1...",subFolderPath)
        destiFileName=os.path.basename(destiFileName)
        #print("2...",destiFileName)
...
def startBackupProcess(backupFileName,logFileName):
    ...
    schedule.every().hour.at("00:01").do(backupSourceFile,backupFileName,backupDirectoryName,logFileNamePath)
    while(True):
