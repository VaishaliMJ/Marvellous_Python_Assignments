...
def initScript():
    try:
        ...
        print("\nRefer /Marvellous_log/DirectoryFileSort_log_{timestamp}.txt in current directory for output or error messages...")
        ...
        if(len(sys.argv)==1):
            ...
            if((sys.argv[1]=="--h") or (sys.argv[1]=="--H")):
                ...
                print("\nRefer DirectoryFileSort_log_{timestamp}.txt for output or error messages...")
            else:  
...
def retrieveExtParticularFiles(directoryName,logFileObj):
      try:  
            fileCount=0
            fileSrcDestPath={}
            makeExtDirectory(directoryName)  
            ...
            for fileName in fileList:
                  ...
                  if(not(fileExtFolder=="")):
                        ...
                        if(os.path.exists(extFolderPath)):
...
def makeExtDirectory(directoryName):
     #C files folder
      cFolder=os.path.join(directoryName,"C")
      if(not(os.path.exists(cFolder))):
            os.mkdir(cFolder)
      #Cpp files folder
      cppFolder=os.path.join(directoryName,"CPP")
      if(not(os.path.exists(cppFolder))):
            os.mkdir(cppFolder)
      #java files folder
      javaFolder=os.path.join(directoryName,"JAVA")
      if(not(os.path.exists(javaFolder))):
            os.mkdir(javaFolder)     
...
def createLogFile(progName,directoryName):
      ...
      try:
            ...
            logFileObj.write(f"\n\t\tSearch files with : c,cpp,.java extension")
