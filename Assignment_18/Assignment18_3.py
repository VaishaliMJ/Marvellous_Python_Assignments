...
def createNewFile(existingFileName,newFileName):
    ...
    copyFileExists,currentWorkingDirectory=Module.checkIfFileExists(existingFileName)
    #print(copyFileExists,"\n\n",currentWorkingDirectory)
    #Check if file to be copied exists or not
    if(copyFileExists):
        #If Copy file exists then check if file is not empty
        isEmpty=Module.checkIfFileNotEmpty(existingFileName)
        if(isEmpty):
...
def checkNewFile(newFileName,existingFileName):
    ...
    newFileExists,currentWorkingDirectory=Module.checkIfFileExists(newFileName)
    if(newFileExists):    
